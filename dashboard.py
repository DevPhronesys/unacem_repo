...
MONTH_ORDER = ["julio", "agosto", "setiembre", "octubre", "noviembre", "diciembre"]
REQUIRED_COLUMNS = {
    "organizacion",
    "local",
    "lat_sede",
...
def attach_computed_fields(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["mes"] = created_at.dt.month.map(MONTH_NAME_ES)
...
def render_filters(df: pd.DataFrame) -> Tuple[List[str], List[str], List[str]]:
    ...
    month_filtered = filter_frame(df, selected_months, [], [])
    available_clients = sorted(month_filtered["organizacion"].unique())
    selected_clients = st.sidebar.multiselect(
        "Cliente (organización)", options=available_clients, default=available_clients
    )

    client_filtered = filter_frame(month_filtered, [], selected_clients, [])
    available_sedes = sorted(client_filtered["local"].unique())
    selected_sedes = st.sidebar.multiselect(
        "Sede / local", options=available_sedes, default=available_sedes
    )
...
def main() -> None:
    ...
    api_url = st.sidebar.text_input(
        ...
        help="Endpoint que devuelve la lista de órdenes en formato JSON.",
    )

    df: pd.DataFrame
    try:
        with st.spinner("Descargando y procesando datos del API..."):
            df_raw = fetch_data(api_url.strip())
            df = attach_computed_fields(df_raw)
    except Exception as exc:  # noqa: BLE001
        st.error(
            "No se pudieron cargar los datos del API. Verifica la URL o la conectividad. "
            f"Detalle: {exc}"
        )
        return

    st.caption("Fuente actual: API")
